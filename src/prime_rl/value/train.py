...
def train_value(config: ValueFunctionConfig) -> None:
    ...
    while config.max_steps is None or value_version < config.max_steps:
        ...
        scale = max(int(global_scale.item()), 1)
        ...
        for reuse_step in range(config.updates_per_batch):
            ...
            for micro_batch in micro_batches:
                ...
                metric_count += metrics["value/abs_error"].numel()
            ...
            metric_totals = torch.stack([loss_sum, abs_error_sum, metric_count, accuracy_sum, accuracy_count])
            dist.all_reduce(metric_totals, op=dist.ReduceOp.SUM, group=dp_group)
            if world.is_master:
                count = max(metric_totals[2].item(), 1.0)
                payload = {
                ...
                if metric_totals[4].item() > 0:
                    payload["value/accuracy"] = metric_totals[3].item() / metric_totals[4].item()
                ...
                logger.info(
                    ...
                    f"loss {metric_totals[0].item():.5f}"
                )
