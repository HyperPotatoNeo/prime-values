...
**RL losses** — `[trainer.loss] type = "default" | "spma" | "ipo" | "custom"`. SPMA uses the bounded linear weight `1 + eta * normalized_advantage`; its default `eta = 0.99` and `reward_range = [0, 1]`. Through the combined `rl` config, selecting SPMA makes an omitted post-batch `zero_advantage` filter monitor-only. Any explicitly configured post-filter list is preserved wholesale, including an explicit enforcing zero filter. Standalone trainer/orchestrator configs do not share this resolver, so set `enforce = false` explicitly there when zero-advantage SPMA samples should train.
